...
struct IMGUI_API ImDrawListSharedData
{
    ...
    Im            CircleSegmentCounts[64];    // Precomputed segment count for given radius before we calculate it dynamically (to avoid calculation overhead)
...
{
    Im        Data[8];        // Can fit any data up to ImGuiDataType_COUNT
};
...
struct ImGuiKeyRoutingData
{
    ...
    Im                            RoutingNextScore;   // Lower is better (0: perfect score)
    ImGuiID                         RoutingCurr;
    ImGuiID                         RoutingNext;
...
struct ImGuiContext
{
    ...
    Im                    DebugLogClipperAutoDisableFrames;
    Im                    DebugLocateFrames;                  // For DebugLocateItemOnHover(). This is used together with DebugLocateId which is in a hot/cached spot above.
    ImS8                    DebugBeginReturnValueCullDepth;     // Cycle between 0..9 then wrap around.
    bool                    DebugItemPickerActive;              // Item picker is active (started with DebugStartItemPicker())
    Im                    DebugItemPickerMouseButton;
...
struct ImGuiTableColumn
{
    ...
    Im                    AutoFitQueue;                   // Queue of 8 values for the next 8 frames to request auto-fit
    Im                    CannotSkipItemsQueue;           // Queue of 8 values for the next 8 frames to disable Clipped/SkipItem
    Im                    SortDirection : 2;              // ImGuiSortDirection_Ascending or ImGuiSortDirection_Descending
    Im                    SortDirectionsAvailCount : 2;   // Number of available sort directions (0 to 3)
    Im                    SortDirectionsAvailMask : 4;    // Mask of available sort directions (1-bit each)
    Im                    SortDirectionsAvailList;        // Ordered list of available sort directions (2-bits each, total 8-bits)
    ...
    ImGuiTableColumn()
    {
        ...
        DrawChannelCurrent = DrawChannelFrozen = DrawChannelUnfrozen = (Im)-1;
    }
};
...
struct ImGuiTableColumnSettings
{
    ...
    Im                    SortDirection : 2;
    Im                    IsEnabled : 1; // "Visible" in ini file
    Im                    IsStretch : 1;
