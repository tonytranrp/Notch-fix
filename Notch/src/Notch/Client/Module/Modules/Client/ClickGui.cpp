#include "ClickGui.h"
#include "../../../../SDK/Attribute/BaseAttributeMap.h"
#include <string>
void getModuleListByCategory(Category category, std::vector<Module*>& modList) {
	for (Module* mod : *moduleMgr->getModuleList()) {
...
ClickGui::ClickGui() : Module("ClickGui", "A GUI use to enable/disable modules.", Category::CLIENT, VK_INSERT) {
	addSlider<float>("Blur", "NULL", &blurStrength, 0.f, 20.f);
	addBoolCheck("Tooltips", "Show modules/settings tooltips.", &tooltips);
	addBoolCheck("GuiMove", "can move while opening gui", &guiMove);
}
...
void ClickGui::onEnable() {
	...
	//DebugConsoleMod->addLog(std::to_string(sizeof(BaseAttributeMap)));
	Game.getClientInstance()->releaseMouse();
}

void ClickGui::onDisable() {
	if (Game.getLocalPlayer() != nullptr) Game.getClientInstance()->grabMouse();
	static DebugConsole* DebugConsoleMod = moduleMgr->getModule<DebugConsole>();
...
void ClickGui::onMouseClickUpdate(int mouseButton, bool isDown, bool& cancel) {
	if (mouseButton == 1) {
	...
	if (isEnabled() && !guiMove) cancel = true;
}

void ClickGui::onKey(int key, bool isDown, bool& cancel) {
	if (!isEnabled()) return;
	if (key == VK_SHIFT) isShiftDown = isDown;
	if (key == VK_BACK) isBackKeyDown = isDown;
	if (!guiMove) cancel = true;
	if (key == VK_ESCAPE) setEnabled(false);
	if (isDown) {
		if (key < 192) {
			lastKeyPress = key;
		...
		if (isSearching) {
			...
			if (key == VK_BACK && !searchingModule.empty()) {
				if (GetAsyncKeyState(VK_CONTROL) & 0x8000) {
					size_t pos = searchingModule.find_last_of(' ');
					if (pos == std::string::npos)
						searchingModule.clear();
					else {
						searchingModule.erase(pos);
					}
				}
				else {
...
void ClickGui::render(ImDrawList* drawlist) {
	...
	Vec2 mousePos = ci->getMousePos();
	ci->releaseMouse();
	//logF(std::to_string(mousePos.x) + " | " + std::to_string(mousePos.y), "INFO");
	...
	openDuration = lerp(1.f, openDuration, io.DeltaTime * 5.f);
	...
	if (isLeftClickDown || isRightClickDown) isSearching = false;
	{
		...
		ImGuiUtils::fillRectangle(Vec4(0.f, 0.f, io.DisplaySize.x, io.DisplaySize.y), bgColor);
		...
		for (ClickWindow* window : windowList) {
			...
			Vec4 cRectPos = Vec4(window->pos.x - textPadding,
			...
			ImGuiUtils::fillRectangle(Vec4(cRectPos.x, cRectPos.w - textPadding * 3.f, cRectPos.z, cRectPos.w), cbgColor);
			ImGuiUtils::fillRectangle(Vec4(cRectPos.x, cRectPos.w - textPadding * 2.f, cRectPos.z, cRectPos.w), UIColor(36, 36, 36, 255));

			float WtextWidth = ImGuiUtils::getTextWidth(window->windowName, textSize * 1.1f);
			Vec2 cTextPos = Vec2(cRectPos.x + (cRectPos.z - cRectPos.x) / 2.f - WtextWidth / 2.f, cRectPos.y);
			...
			if (window->extended) {
				...
				for (Module* mod : window->modList) {
					...
					Vec4 mRectPos = { xOffset, yOffset, xEnd, yOffset + textHeight + textPadding * 2.f };
					Vec4 mRectPos2 = { mRectPos.x + 1.f, mRectPos.y + 1.f, mRectPos.z - 1.f, mRectPos.w - 1.f };
					...
					Vec2 mTextPos = { mRectPos.x + textPadding * 4.f, mRectPos.y + textPadding };
					ImGuiUtils::drawText(
						mTextPos,
						mod->getModuleName(),
					...
					if (mod->extended) {
						...
						for (Setting* setting : *mod->getSettings()) {
							...
							Vec4 sbgRectPos = Vec4(mRectPos.x, yOffset, mRectPos.z, yOffset + textHeight + textPadding * 2.f);
							Vec4 sRectPos = Vec4(sbgRectPos.x + textPadding * 5.f, sbgRectPos.y, sbgRectPos.z - textPadding * 5.f, sbgRectPos.w - textPadding);
							Vec2 sTextPos = Vec2(sRectPos.x + textPadding * 4.f, sRectPos.y + textPadding);
							...
							case SettingType::KEYBIND: {
								...
								ImGuiUtils::drawText(Vec2(sRectPos.z - ImGuiUtils::getTextWidth(textStr, textSize) - textPadding * 4.f, sTextPos.y), textStr, whiteColor, textSize, true);
								yOffset += textHeight + textPadding * 2.f;
								break;
							}
							case (SettingType::BOOL): {
								if (setting->valuePtr->_bool) {
									drawlist->AddRectFilledMultiColor(ImVec2(sRectPos.x, sRectPos.y), ImVec2(sRectPos.z, sRectPos.w), mainColor.toImColor(), ImColor(0, 0, 0, 0), ImColor(0, 0, 0, 0), mainColor.toImColor());
								}
							...
							case (SettingType::ENUM): {
								...
								ImGuiUtils::drawText(Vec2(sRectPos.z - ImGuiUtils::getTextWidth(enumName, textSize) - textPadding * 4.f, sTextPos.y), enumName, whiteColor, textSize, true);
								yOffset += textHeight + textPadding * 2.f;
								break;
							}
							case (SettingType::COLOR): {
								ImGuiUtils::drawText(sTextPos, setting->settingName + ":", whiteColor, textSize, true);
								Vec4 colorRectPos(sRectPos.z - 20.f, sRectPos.y + 5.f, sRectPos.z - 5.f, sRectPos.w - 5.f);
								...
								if (setting->extended) {
									Vec4 pickerPos(sbgRectPos.x, sbgRectPos.w, sbgRectPos.z, sbgRectPos.w + (textHeight + textPadding * 2.f) * 4.f);
									ImGuiUtils::fillRectangle(pickerPos, mbgColor);
									static const Vec2 colorPickerSize = Vec2(136.f, 100.f);
									Vec4 colorPickerPos(pickerPos.x + 5.f, pickerPos.y + 5.f, pickerPos.x + 5.f + colorPickerSize.x, pickerPos.y + 5.f + colorPickerSize.y);
									...
									// Draw dot
									Vec4 dotPos = Vec4(setting->pos.x + colorPickerPos.x, setting->pos.y + colorPickerPos.y, setting->pos.x + colorPickerPos.x + 3.f, setting->pos.y + colorPickerPos.y + 3.f);
									...
									Vec4 huePickerRect(colorPickerPos.z + 5.f, colorPickerPos.y, colorPickerPos.z + barWidth + 5.f, colorPickerPos.y + barHeight);
									...
									ImGuiUtils::fillRectangle(Vec4(huePickerRect.x, posY - 1.f, huePickerRect.z, posY + 1.f), whiteColor);

									Vec4 opacityPickerRect(huePickerRect.z + 5.f, colorPickerPos.y, huePickerRect.z + barWidth + 5.f, colorPickerPos.y + barHeight);
									...
									ImGuiUtils::fillRectangle(Vec4(opacityPickerRect.x, posY2 - 1.f, opacityPickerRect.z, posY2 + 1.f), whiteColor);
							...
							case (SettingType::SLIDER): {
								...
								if (valueType == ValueType::INT_T) {
									...
									ImGuiUtils::drawText(sTextPos, setting->settingName + ":", whiteColor, textSize, true);
									ImGuiUtils::drawText(Vec2(sRectPos.z - ImGuiUtils::getTextWidth(std::to_string(setting->valuePtr->_int), textSize) - textPadding * 4.f, sTextPos.y), std::to_string(setting->valuePtr->_int), whiteColor, textSize, true);
									...
									ImGuiUtils::drawText(Vec2(sRectPos.z - ImGuiUtils::getTextWidth(std::string(str), textSize) - textPadding * 4.f, sTextPos.y), (std::string(str)), whiteColor, textSize, true);
				...
				if (Vec4(window->pos.x, window->pos.y, window->pos.x + windowWidth, window->pos.y + io.DisplaySize.y * 0.65f).contains(&mousePos)) {
			...
			if (yOffset < window->pos.y + io.DisplaySize.y * 0.65f) ImGuiUtils::drawRectangle(Vec4(cRectPos.x, cRectPos.y, cRectPos.z, yOffset), UIColor(36, 36, 36, 255), 2.f, roundValue, ImDrawFlags_RoundCornersTop);
			else ImGuiUtils::drawRectangle(Vec4(cRectPos.x, cRectPos.y, cRectPos.z, window->pos.y + io.DisplaySize.y * 0.65f), UIColor(36, 36, 36, 255), 2.f, roundValue, ImDrawFlags_RoundCornersTop);
		}
	}

	if (this->tooltips && tooltipString != std::string("NULL")) {
		Vec2 textPos(mousePos.x + 5.f, mousePos.y + 20.f);
		Vec4 rectPos(textPos.x - 5.f, textPos.y, textPos.x + ImGuiUtils::getTextWidth(tooltipString, 0.9f) + 5.f, textPos.y + ImGuiUtils::getTextHeight(0.9f));
		ImGuiUtils::fillRectangle(rectPos, UIColor(21, 21, 21, 255), roundValue);
		ImGuiUtils::drawRectangle(rectPos, UIColor(36, 36, 36, 255), 1.f, roundValue);
		ImGuiUtils::drawText(textPos, tooltipString, whiteColor, 0.9f);
	}

	Vec4 searchRegion = Vec4(io.DisplaySize.x / 2.f - 275.f,
	...
	if (searchRegion.contains(&mousePos)) {
		searchDuration = lerp(1.f, searchDuration, io.DeltaTime * 10.f);
		closeDuration = 3.f;
	}
	else {
		if (!isSearching && searchingModule.empty()) {
			if (closeDuration < 0.f) searchDuration = lerp(0.f, searchDuration, io.DeltaTime * 10.f);
			else closeDuration -= io.DeltaTime;
		}
	}
	static const std::string SearchStr = "Search";
	Vec4 searchRectPos = Vec4(io.DisplaySize.x / 2.f - searchWidth / 2.f,
		io.DisplaySize.y - searchHeight / 2.f + 10.f - 50.f * searchDuration,
		io.DisplaySize.x / 2.f + searchWidth / 2.f,
		io.DisplaySize.y + searchHeight / 2.f + 10.f - 50.f * searchDuration);
	Vec4 typeRectPos = Vec4(searchRectPos.x + 5.f,
		searchRectPos.y + 5.f,
	...
	Vec2 typeTextPos = Vec2(typeRectPos.x + 5.f, typeRectPos.y + textPadding * 2.f);
	...
	ImGuiUtils::drawText(Vec2(typeRectPos.z + 5.f, typeTextPos.y), SearchStr, whiteColor, textSize);
	...
	barXAdd = lerp(textWidth, barXAdd, io.DeltaTime * 7.5f);
	Vec4 barRect = Vec4(typeTextPos.x + barXAdd,
		mid - (textSize * 9.f),
