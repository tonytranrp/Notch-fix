...
AutoEmote::AutoEmote() : Module("AutoEmote", "Auto-doing Random Emote", Category::PLAYER) {
	addSlider<int>("Emote delay", "NULL", &EmoteDelay, 0, 20);
}
...
void AutoEmote::onTick(LocalPlayer* actor) {
	...
	if (Emotedelaytick >= EmoteDelay) {
		...
		actor->playEmote(emoteUUIDs[currentEmoteIndex], false);
		currentEmoteIndex++;
		Emotedelaytick = 0;
	}
